### 
### read data.txt to extract single-valued irreps
###
...
# a dict for each point group: {  single-valued irrep names: xxx, symm_op names: xxx, \
#				            single-values irreps : [ [xx], [xx], ...., [xx] ]  } 
# ===================================================================================
...
# try dealing with single-valued irrep names ====================================
	...
	for dd in data[0] :
		if 'GM' in dd :
			...
			if '<sup>+</sup>' in dd :
				...
				sup = '-'
			prop_tmp = re.findall( r'\((.+?)\)', dd )
			prop_tmp = prop_tmp[0]
			assert len( prop_tmp ) == 1, prop_tmp
			prop = prop_tmp
			index = ''.join( index )
			name = 'GM' + index + sup + '(' + prop + ')'
			names.append( name )
	all_32_datas[ipg-1]['single_irrep_names'] = names
	#print( '\n' )

#for da in all_32_datas :
#	print( da )
#print( '\n' )
#print( all_32_datas[18] )
...
for ipg in range( 1, 33 ) :
	...
	for k in range( 1, int((len(data)+1)/2) ) :
		...
		sym_ops.append( { 'index': index, 'pm': pm, 'axis': axis, 'mat': mat } )
...
#for xx in all_32_datas[18]['sym_ops']:
#	print(xx)
#	print('\n')
...
### deal with all sinlge-valued irreps 
...
for ipg in range( 1, 33 ) :
	...
	## find the begining of single-valued irreps 
	begin_sv_irrep = 0
	end_sv_irrep = 0
	find_begin = False
	for i in range( len(data[0]) ):
		dd = data[0][i]
		if 'GM' in dd and 'overline' not in dd :
			begin_sv_irrep = i
			find_begin = True
			break
	assert begin_sv_irrep == 3
	for i in range( len(data[0]) ):
		dd = data[0][i]
		if 'GM' in dd and 'overline' in dd :
			end_sv_irrep = i
			find_end = True
			break
	assert begin_sv_irrep == 3
	...
	for k in range( 1, int((len(data)+1)/2) ) :
		sv_irreps_symmop = [ ]
		sv_irreps_symmop_str = []
		#print( 'begin_sv_irrep, len_data_k :', begin_sv_irrep, len(data[k]) )
		#print( 'data[k][-1]:', data[k][-1] )
		for m in range( begin_sv_irrep+1, end_sv_irrep+1 ) : 
	#for m in range( 4, len(data[1]) ) :
		#len_bb = 0
		#for k in range( 1, int((len(data)+1)/2) ) : 
			strs = data[k][m]
			...
			sv_irreps_symmop_str.append( mat_elemts_str )
			sv_irreps_symmop.append( mat_elemts )
			#print( '\n' )
		sv_irreps_str.append( sv_irreps_symmop_str )
		sv_irreps.append( sv_irreps_symmop )
		#print( '\n' )
	#print( '\n' )
	#all_32_datas[ipg-1]['sv_irreps_str'] = sv_irreps_str
	all_32_datas[ipg-1]['sv_irreps'] = sv_irreps

print( 'import numpy as np' )
print( 'all_sv_irreps = ', all_32_datas )
#print( all_32_datas[18] )
